...
// if the score numbers is divisible by 3, 
...
const assignFizzTrick = function(explorer){
    if(explorer.score%3 === 0){//comprueba si score es divisible entre 3
        explorer.trick = "FIZZ";//si es verdadero se agrega la propiedad trick y se le asigna FIZZ
        return explorer;
    }else{
...
const assignBuzzTrick = function(explorer){
    if(explorer.score%5 === 0){//comprueba si score es divisible entre 5
        explorer.trick = "BUZZ";//si es verdadero se agrega la propiedad trick y se le asigna BUZZ
        return explorer;
    }else{
        explorer.trick = explorer.score;
        return explorer;
...
const assignFizzBuzzTrick = function(explorer){
    if(explorer.score%5 === 0 && explorer.score%3 === 0){//comprueba que score sea divisible entre 5 y 3
        explorer.trick = "FIZZBUZZ";//agrega la propiedad FIZZBUZ a trick
        return explorer;
    }else{
        explorer.trick = explorer.score;//agrega el valor de score a trick
        return explorer;
    }
};
...
//trcik and the value BUZZ, if is just divisible by 3 set the property trick and the value FIZZ, 
...
const explorersInNodeAndFizzBuzz=explorersInNode.map((explorer)=> assignFizzBuzzTrickBuzz(explorer));
console.log(explorersInNodeAndFizzBuzz);